...
service::ResolverMap Subscription::getResolvers() const noexcept
{
	return {
		...
		{ R"gql(subscriptionTelemetry)gql"sv, [this](service::ResolverParams&& params) { return resolveSubscriptionTelemetry(std::move(params)); } }
	};
}
...
service::AwaitableResolver Subscription::resolveSubscriptionTelemetry(service::ResolverParams&& params) const
{
	std::unique_lock resolverLock(_resolverMutex);
	service::SelectionSetParams selectionSetParams { static_cast<const service::SelectionSetParams&>(params) };
	auto directives = std::move(params.fieldDirectives);
	auto result = _pimpl->getSubscriptionTelemetry(service::FieldParams { std::move(selectionSetParams), std::move(directives) });
	resolverLock.unlock();
...
void AddSubscriptionDetails(const std::shared_ptr<schema::ObjectType>& typeSubscription, const std::shared_ptr<schema::Schema>& schema)
{
	typeSubscription->AddFields({
		schema::Field::Make(R"gql(subscriptionTelemetry)gql"sv, R"md()md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType(R"gql(Telemetry)gql"sv)))
	});
}
